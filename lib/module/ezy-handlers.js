...
class EzyConnectionFailureHandler {
  handle(event) {
    ...
    var reasonName = Const.EzyConnectionFailedReasons.getConnectionFailedReasonName(reason);
...
class EzyDisconnectionHandler {
  handle(event) {
    ...
    var mustReconnect = reconnectEnable && reason !== Const.EzyDisconnectReason.UNAUTHORIZED && reason !== Const.EzyDisconnectReason.CLOSE && should;
...
class EzyEventHandlers {
  ...
  handle(eventType, data) {
    ...
    if (handler) handler.handle(data);else EzyLogger.warn('has no handler with event: ' + eventType);
  }
...
class EzyAppResponseHandler {
  handle(data) {
    ...
    if (handler) handler(app, commandData);else EzyLogger.warn('app: ' + app.name + ' has no handler for command: ' + cmd);
  }
...
class EzyPluginResponseHandler {
  handle(data) {
    ...
    if (handler) handler(plugin, commandData);else EzyLogger.info('plugin: ' + plugin.name + ' has no handler for command: ' + cmd);
  }
...
class EzyDataHandlers {
  ...
  handle(cmd, data) {
    ...
    if (handler) handler.handle(data);else EzyLogger.warn('has no handler with command: ' + cmd);
  }
